...
- Added test to confirm new sources are present: `backend/tests/test_whistleblower_v2_sources.py`.
- Automated outcome: sources include LinkedIn, Facebook, and TikTok links; full test suite passes.
- Human outcome: UI shows additional sources under Assessment; still adheres to non-allegation policy when signals are absent.
- Agentic deep search: validated parallel fetch with a capped limit (20) and timeouts; in production, enable by passing `deep_search=true` and optional `limit`.