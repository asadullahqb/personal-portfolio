...
def instruct_get_localized_welcome_from_ip(ip: str) -> Dict[str, str]:
    ...
    prompt = (
        f"Given IP {ip}, return JSON with: "
        f"country_code (2-letter), language (2-letter code), "
        f"message (localized welcome in that language)"
    )
    payload = {
        "messages": [
            {
    ...
    try:
        ...
        if hf_payload is None:
             return {
            "language": "en", "message": "Welcome.", "country_code": "US", "source": "fallback"
        }
...
def get_welcome_message(ip: str) -> Dict[str, Any]:
    """
    Returns a localized welcome message based on IP address.
    """
